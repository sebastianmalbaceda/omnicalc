      ...
      <div className="flex-1 flex flex-col gap-4">
        ...
        <div className="flex flex-row gap-3">
          {['sin', 'cos', 'tan', 'log', 'ln', 'sqrt'].map((fn) => (
            <button
              key={fn}
              onClick={() => isPro && handleScientificOperation(fn as ScientificFunction)}
              disabled={!isPro}
              className={`btn btn-function flex-1 ${!isPro && 'opacity-40'}`}
            >
        ...
        <div className="flex flex-row gap-3">
          {[
            ...
            <button
              ...
              onClick={item.action}
              disabled={
                !isPro && (item.label === 'x²' || item.label === 'xⁿ' || item.label === '1/x')
              }
              className={`btn btn-function flex-1 ${!isPro && 'opacity-40'}`}
            >
